...
FREIGHT_DEFAULTS = {
    ...
    ("Китай", "Nansha", "Новороссийск", "20"): 4350.0,
    ("Китай", "Nansha", "Санкт-Петербург", "20"): 4950.0,
    ("Китай", "Nansha", "Владивосток", "20"): 2750.0,
    ...
    ("Китай", "Nansha", "Новороссийск", "40"): 5950.0,
    ("Китай", "Nansha", "Санкт-Петербург", "40"): 6650.0,
    ("Китай", "Nansha", "Владивосток", "40"): 3550.0,
...
def calc_model(
    ...
    # Сначала считаем себестоимость в RUB/м², чтобы использовать её в расчёте USD/м²
    total_rub_all = (total_usd_all * usd_rub) + local_rub
...
# Себестоимость USD/м² (новая логика):
    ...
    cost_all_usd_m2 = (cost_all_rub_m2 / usd_rub) if qty_m2 else 0
    cost_all_rub_m2 = total_rub_all / qty_m2 if qty_m2 else 0
...
with st.sidebar:
    ...
    with c1:
        if country == "Индия":
            ...
            port_loading = st.selectbox("Порт отгрузки", ["Qingdao", "Shanghai", "Nansha", "Foshan"])
    ...
    if "_prev_local_costs_detail_sum" not in st.session_state:
        # первый запуск
        st.session_state["_prev_local_costs_detail_sum"] = _cur_detail_sum
        st.session_state["local_costs_rub_input"] = _cur_detail_sum
    else:
        _prev_detail_sum = float(st.session_state.get("_prev_local_costs_detail_sum", _cur_detail_sum))
        # сравнение с округлением, чтобы не ловить "шум" float
        if round(_cur_detail_sum, 6) != round(_prev_detail_sum, 6):
            st.session_state["_prev_local_costs_detail_sum"] = _cur_detail_sum
            st.session_state["local_costs_rub_input"] = _cur_detail_sum
    ...
    lr_storage = st.number_input(
        "Таможенное оформление груза в Р.Ф., RUB/декл.",
...
if calc:
    ...
    _print_rows_left = [
        ...
        ("Порт выгрузки", port_discharge),        ("Курс на дату платежа USD→RUB(в т.ч ЦБ+2,5%)", _fmt_money(currency_rate, 2)),
        ("Инвойс (итого)", f"{_fmt_money(invoice_total, 2)} {invoice_currency}"),
        ("Товар", product_type),
        ("Поверхность", finish),
        ("Код ТН ВЭД", hs_code),
        ("Пошлина, %", f"{_fmt_money(duty_pct, 2)}%"),
        ("НДС, %", f"{_fmt_money(VAT_PCT_FIXED, 2)}%"),
    ...
    _print_totals = [
        ...
        ("Локальные расходы в РФ (после порта), всего", local_costs_rub_input, "₽"),
...
<body>
  ...
  <div class="grid">
    <div class="left">
      ...
      <div class="box">
        <h3>Расшифровка статьи "Локальные расходы (после порта) в РФ, всего"</h3>
        <table class="t">
          {_rows_local_html}
        </table>
      </div>
...
Срок действия: Расчет предварительный и действителен на дату составления (05.02.2026).

Валютные риски: Рублевая стоимость фиксируется в момент оплаты. При изменении курса валют на дату платежа сумма пересчитывается. При частичной оплате финальная стоимость складывается из суммы всех траншей по курсам на дни оплат.

Корректировка фрахта: Ставка фрахта индикативная и может быть пересмотрена по тарифам линии на момент букинга.

Дополнительные расходы: В расчет НЕ включены возможные сборы, возникающие по требованию контролирующих органов или вследствие простоя:

МИДК/ИДК (рентген-сканирование контейнера);

Таможенный досмотр/осмотр и взвешивание;

Сверхнормативное хранение (Storage) и использование контейнера (Demurrage/Detention) в порту и на СВХ.

Данные расходы оплачиваются отдельно по факту выставления счетов терминалом/линией.
        </p>
      </div>
    </div>
  </div>
