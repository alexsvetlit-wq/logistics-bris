...
if calc:
    ...
    _print_local_detail = [
        ...
        ("Перетарка на СВХ кросс-докинг (КТК → авто) от", lr_restack_cross, "₽"),
        ("ПРР механизированная (из ктк -склад- авто), RUB/паллет от", lr_prr_mech, "₽"),
        ("ПРР ручная (из ктк авто/склад) за 1 тн без паллеты, RUB/тонна от", lr_prr_manual, "₽"),
        ("Паллетированние комплекс(вкл.поддон+стрейч+пп лента), RUB/паллет от", lr_restack_ktt, "₽"),
        ("Перетарка на СВХ (с ктквоз снять/поставить), RUB/ктк лифт от", lr_restack_terminal, "₽"),
        ("Хранение на СВХ (1 под/сутки начиная с 10 дня), RUB/паллетодень от", lr_storage, "₽"),
