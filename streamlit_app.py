...
if calc:
    ...
    _print_local_detail = [
        ("Вывоз КТК из порта на СВХ (в т.ч. депо) от", lr_ktt_out, "₽"),
        ("Перетарка СВХ кросс-докинг (КТК → авто) от", lr_restack_cross, "₽"),
        ("ПРР механизированная", lr_prr_mech, "₽") от,
        ("ПРР ручная", lr_prr_manual, "₽") от,
        ("Паллетирование комплекс", lr_restack_ktt, "₽") от,
        ("Перетарка на СВХ (лифт)", lr_restack_terminal, "₽") от,
        ("Хранение на СВХ", lr_storage, "₽") от,
        ("Доставка по РФ до склада клиента", lr_delivery_rf, "₽") от,
    ]
